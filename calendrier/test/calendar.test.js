...
test('la grille fait 7 jours et une ligne par heure possible', async () => {
  ...
  assert.deepEqual(grille.times, TIMES);
  ...
  assert.deepEqual(grille.rows.map((r) => r.time), TIMES, 'les lignes suivent l’ordre du référentiel');
...
test('chaque séance tombe dans la bonne cellule jour x heure', async () => {
  ...
  await svc.create({ date: '2026-09-11', time: '10:30', locationId: 'valbonne-stadium', title: 'Matin' });

  const grille = buildCalendar(svc, { start: '2026-09-11' });
  const ligne = (heure) => grille.rows.find((r) => r.time === heure);
  const ligne1030 = ligne('10:30');
  const ligne1800 = ligne('18:00');
  const ligne1830 = ligne('18:30');

  assert.equal(grille.total, 4);
  assert.equal(ligne1030.cells[0].sessions.length, 1, 'le créneau du matin a sa propre ligne');
  assert.equal(ligne1030.cells[0].sessions[0].title, 'Matin');
  assert.equal(ligne1030.cells[1].sessions.length, 0);
...
test('une cellule est complète quand les 5 lieux sont pris', async () => {
  ...
  const cellule = grille.rows.find((r) => r.time === '18:00').cells[0];
